...
# 12D Clock Ratios for maximum resonance
CLOCK_RATIOS_6D = {
    'golden': PHI,
    'silver': 1 + np.sqrt(2),
...
CLOCK_RATIOS_12D = {
    **CLOCK_RATIOS_6D,
    ...
    'copper': PHI + 1,
    'nickel': np.sqrt(3),
    'aluminum': (1 + np.sqrt(2)) / 2,
    'titanium': 2 ** (1/3),
    'chromium': (1 + np.sqrt(13)) / 2,
}


class LazyClockOracle:
    """
    Lazy clock phase oracle with memoization for 145× speedup.
    
    Supports both 6D and 12D tensor resonance.
    """
    
    def __init__(self, use_memoization: bool = True, max_n: int = 10000, use_12d: bool = True):
        """
        ...
        Args:
            ...
            max_n: Maximum index to precompute
            use_12d: If True, use 12D tensor
        """
    ...
    def get_6d_tensor_phase(self, n: int) -> np.ndarray:
        """Get 6D tensor of phases."""
        return np.array([self.get_fractional_phase(n, name) for name in CLOCK_RATIOS_6D])
    
    def get_12d_tensor_phase(self, n: int) -> np.ndarray:
        """Get 12D tensor of phases."""
        return np.array([self.get_fractional_phase(n, name) for name in CLOCK_RATIOS_12D])
